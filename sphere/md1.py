...
resolution = 380  # Resolução da imagem
x_min, x_max, y_min, y_max = -3, 3, -3, 3  # Intervalos dos eixos x e y
max_iter = 250  # Número máximo de iterações para o cálculo do Mandelbrot

# Função que calcula o número de iterações para um ponto c no conjunto de Mandelbrot
@jit(nopython=True)
def mandelbrot_scalar(c, max_iter):
    z = 0  # Inicializa z com 0
    n = 0  # Contador de iterações
    # Itera até que o valor absoluto de z seja maior que 2 ou até atingir o número máximo de iterações
...
# Função que calcula o conjunto de Mandelbrot e suas projeções em uma esfera
@jit(nopython=True, parallel=True)
def calculate_mandelbrot_set(Z, max_iter, resolution):
    ...
    for i in prange(resolution):
        for j in prange(resolution):
            iter_count = mandelbrot_scalar(Z[i, j], max_iter)  # Calcula o número de iterações para o ponto Z[i, j]
...
# Calcula o conjunto de Mandelbrot e suas projeções
a, b, c, colors = calculate_mandelbrot_set(Z, max_iter, resolution)
...
# Plota os pontos do conjunto de Mandelbrot na superfície da esfera
scatter = ax.scatter(a.flatten(), b.flatten(), c.flatten(), c=colors.flatten(), cmap='hot', s=1.5)
...
ax.set_ylabel('Y')
ax.set_zlabel('Z')
ax.set_title('Conjunto de Mandelbrot projetado em uma esfera com gradiente de cores')
